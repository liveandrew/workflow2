...
public class TestWorkflowRunner extends JavaSupportTestCase {
  ...
  public void testLoneMultiStepAction() throws Exception {
    // lone multi
    Step s = new Step(new MultiStepAction("lone", Arrays.asList(new Step(new IncrementAction("blah")))));
  ...
  public void testMultiInTheMiddle() throws Exception {
    ...
    s = new Step(new MultiStepAction("lone", Arrays.asList(new Step(new IncrementAction("blah")))), s);
  ...
  public void testMultiInMultiEnd() throws Exception {
    ...
    s = new Step(
        new MultiStepAction("depth 1",
            Arrays.asList(
                new Step(
                    new MultiStepAction("depth 2",
                        Arrays.asList(
                            new Step(new IncrementAction("blah"))))))), s);
    s = new Step(new IncrementAction("last"), s);
    new WorkflowRunner("", checkpointDir, 1, null, s).run();
    assertEquals(3, IncrementAction.counter);
  }
  ...
  public void testMulitInMultiMiddle() throws Exception {
    ...
    Step innermost = new Step(new MultiStepAction("innermost", Arrays.asList(new Step(new IncrementAction("c")))), b);
  ...
  public void testDuplicateCheckpoints() throws Exception {
    try {
      new WorkflowRunner("", checkpointDir, 1, null, new Step(new IncrementAction("a")), new Step(new IncrementAction("a")));
