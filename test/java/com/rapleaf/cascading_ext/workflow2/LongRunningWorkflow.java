...
public class LongRunningWorkflow extends JavaSupportTestCase {
  ...
  public void testIt() throws IOException {
    ...
    Step s2 = new Step(new ExampleAction("Step 2", new DataStore[0], new DataStore[]{d1, d2}), s1);
    Step s3 = new Step(new ExampleAction("Step 3", new DataStore[0], new DataStore[]{d3}), s1);

    Step s4_1 = new Step(new ExampleAction("sub-step 1", new DataStore[]{d1}, new DataStore[]{d1, id1}));
    Step s4_2 = new Step(new ExampleAction("sub-step 2", new DataStore[]{d2}, new DataStore[]{id2}));
    Step s4_3 = new Step(new ExampleAction("sub-step 3", new DataStore[]{d1, id1, id2}, new DataStore[]{d4}), s4_1, s4_2);
    Step s4 = new Step(new ExampleMultistepAction("Multistep-1", new Step[]{s4_1, s4_2, s4_3}), s2);

    Step s5_1_1 = new Step(new ExampleAction("sub-sub-step 1", new DataStore[]{d2, d3}, new DataStore[]{d3}));
    Step s5_1_2 = new Step(new TakeSomeTime(0, "sub-sub-step 2", new DataStore[]{d3}, new DataStore[]{id3}), s5_1_1);
    Step s5_1 = new Step(new ExampleMultistepAction("sub-multistep", new Step[]{s5_1_1, s5_1_2}));

    Step s5_2 = new Step(new ExampleAction("sub-step 2", new DataStore[]{d3}, new DataStore[]{id4}), s3);
    Step s5_3 = new Step(new ExampleAction("sub-step 3", new DataStore[]{id4}, new DataStore[]{d6}), s5_2);
    Step s5_4 = new Step(new ExampleAction("sub-step 4", new DataStore[]{id3, id4}, new DataStore[]{d5}), s5_1, s5_2);
    Step s5 = new Step(new ExampleMultistepAction("Multistep-2", new Step[]{s5_1, s5_2, s5_3, s5_4}), s2, s3);

    Step s6 = new Step(new FailBang(new DataStore[]{d5, d6}, new DataStore[]{d7}), s5);
    Step s7 = new Step(new TakeSomeTime(5, "Step 7", new DataStore[]{d1, d4, d7}, new DataStore[0]), s4, s6);
    
    getFS().delete(new Path(LONG_RUNNING_WORKFLOW_PATH), true);
    WorkflowRunner wfr = new WorkflowRunner("Long Running Test Workflow", LONG_RUNNING_WORKFLOW_PATH, 3, 34627, s7);
