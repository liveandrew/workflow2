...
import com.rapleaf.support.workflow2.WorkflowDiagram.Vertex;

public class TestWorkflowDiagram extends JavaSupportTestCase {
  ...
  private Step getComplexNestedWorkflowTail() throws Exception {
    ...
    Step s4_3 = new Step(new FakeAction("3", new DataStore[] {d1, id1, id2}, new DataStore[] {d4}), s4_1, s4_2);
    ...
    Step s5_1_2 = new Step(new FakeAction("2", new DataStore[] {d3}, new DataStore[] {id3}), s5_1_1);
    ...
    Step s5_4 = new Step(new FakeAction("4", new DataStore[] {id3, id4}, new DataStore[] {d5}), s5_1, s5_2);
    ...
    Step s7 = new Step(new FakeAction("s7", new DataStore[] {d1, d4, d7}, new DataStore[0]), s4, s6);
  ...
  private WorkflowDiagram getWorkflowDiagramFromTails(Step first, Step... rest) {
    WorkflowRunner wfr = new WorkflowRunner("Test Workflow", getTestRoot() + "/test_workflow", 3, 12345, first, rest);
  ...
  private void verifyNumVertices(int expectedNumVertices) {
    assertEquals("Wrong number of vertices in workflow graph.",
        expectedNumVertices, graph.vertexSet().size());
  }
  
  private void verifyNumEdges(int expectedNumEdges) {
    assertEquals("Wrong number of edges in workflow graph.",
        expectedNumEdges, graph.edgeSet().size());
  }
